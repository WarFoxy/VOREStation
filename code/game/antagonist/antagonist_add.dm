...
/datum/antagonist/proc/add_antagonist_mind(var/datum/mind/player, var/ignore_role, var/nonstandard_role_type, var/nonstandard_role_msg)
	...
	spawn(1 SECOND) //Added a delay so that this should pop up at the bottom and not the top of the text flood the new antag gets.
		to_chat(player.current, "<span class='notice'>Как только вы определитесь с целью, которую будете преследовать, вы можете дополнительно отобразить ее на экране. \
			для всех в конце раунда, использую <b>Set Ambition</b> в панели IC.  Вы можете это в любое время, \
			да и в противном случае, это не имеет никакого отношения к вашему раунду.</span>")
