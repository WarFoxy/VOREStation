...
/client/proc/respawn_character_proper(client/picked_client)
	...
	if(record_found)
		var/samejob = alert(src,"Found [picked_client.prefs.real_name] in data core. They were [record_found.fields["real_rank"]] this round. Assign same job? They will not be re-added to the manifest/records, either way.","Previously spawned","Yes","Ассистент","No")
		if(samejob == "Yes")
